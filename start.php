<?php
	...
	function karma_update($guid, $obs_score , $commit ) {
		...
		$karma->marketing_score = array($old_marketing_score[0] + 
			$twitter_score, $old_marketing_score[1] + $planet_opensuse_score);
		
		$karma->activity = array($num_of_tweets + $old_activity[0],
			$num_of_bugs_fixed,$num_of_posts + $old_activity[2], $commit
			+ $old_activity[3], $num_of_edits + $old_activity[4]);	
		
		$karma->total_score = $karma->developer_score[0] + 
	...
	function wiki_score($username, $guid) {
		...
		foreach ($locales as $locale) {
			...
			$url = "http://$locale.opensuse.org/index.php?title=Special:
				Contributions/".$username."&feed=atom&deletedOnly=&limit=
				10&target=".$username."&topOnly=&year=&month=";
	...
	function calculate_max_score($developer_score, $marketing_score) {
		 ...
		 * argument and previously existing maximum score.*/
			...
			$max_karma->max_developer = max($max_karma->max_developer,
				$developer_score);
			$max_karma->max_marketing = max($max_karma->max_marketing,
				$marketing_score);
		}
		$max_karma->save();
	...
	function assign_badge($developer, $marketing, $wiki_score, $max_score) {
		...
		if($bugzilla_score == 0 && $planet_opensuse_score == 0 && 
			$twitter_score == 0 && $build_service_score == 0 && $wiki_score == 0)
				$badge = "Novice";

		else if ($developer_score >= $marketing_score && $developer_score 
			>= $wiki_score) {
				if ($developer_score == $max_developer)
						$badge = "Numero Uno";
				else if ($developer_score >= 0.85 * $max_developer && 
					$developer_score <= $max_developer)
						$badge = "Elite Stallion";
			else if ($developer_score >= 0.75 * $max_developer && 
				$developer_score < 0.85 * $max_developer)
					$badge = "SUSE Samurai";
			else if ($developer_score >=0.5 * $max_developer && 
				$developer_score < 0.75 * $max_developer) 
					$badge = "Citizen Patrol";
			else if ($developer_score >= 0.40 * $max_developer && 
				$developer_score < 0.5 * $max_developer){
		...
		else if ($marketing_score > $developer_score && $marketing_score 
			> $wiki_score) {
				if ($marketing_score == $max_marketing)
					$badge = "SUSE Herald";
				else if ($marketing_score >= 0.75 * $max_marketing && 
					$marketing_score < $max_marketing)
						$badge = "Autobiographer";
				else if ($marketing_score >= 0.5 * $max_marketing && 
					$marketing_score < 0.75 * $max_marketing)
						$badge = "Enthusiast";
				else {
	...
	function karma_permissions_check($hook_name, $entity_type, 
	...
	function load_badge_suggestion($badge) {
			if ($badge == "Twitteratti")
				$message = "Your badge suggests that Tweeting has been 
				your only significant contribution to the openSUSE society. 
				We strongly believe that you could do more and have fun!";
			else if ($badge == "Blog-o-Manic")
				$message = "Your badge suggests that blogging has been your 
				only significant contribution to the openSUSE society";
			else if ($badge == "Enthusiast")
				$message = "Your badge suggests that you a quite a marketier, 
				you blog and tweet quite often, but we feel you should do 
				more of this. Have fun!";
			else if ($badge == "Autobiographer")
				$message = "Your badge suggests that you are an avid blogger 
				or twitteratti, we would really like you to keep up the great 
				work and have fun!";	
			else if ($badge == "SUSE Herald")
				$message = "Your badge suggests that you are a top-scorer. 
				You have the maximum marketing Karma and we are proud of you!";
			else if ($badge == "Notable Endeavor")
				$message = "Your badge suggests that you are involved more 
				with developer works under openSUSE, but your contribution needs 
				to be more significant. Keep up the good work and have fun!";
			else if ($badge == "Build Superhero")
				$message = "Your badge suggests that you have been working 
				with various distributions on build service, we think that 
				is great! But we also feel you can contribute more to openSUSE 
				to make your contribution much more significant";
			else if ($badge == "Bug Buster")
				$message = "Your badge suggests that you have been fixing 
				bugs on Novell's Bugzilla, we think that is great! But we 
				also feel you can contribute more to openSUSE to make your 
				contribution more signifcant";
			else if ($badge == "Citizen Patrol")
				$message = "Your badge suggests that you are quite a developer! 
				You fix bugs or make quite many build service commits, 
				and we encourage you to do more of this, have fun!";
			else if ($badge == "SUSE Samurai")
				$message = "Your badge suggests that you significantly 
				contribute to openSUSE, in terms of fixing bugs and working 
				with distributions on build service. We are proud of you 
				and we feel, that you do more of this and have fun! ";
			else if ($badge == "Elite Stallion")
				$message = "Your badge suggests that you are a core developer, 
				we salute you and encourage you to keep up the good work 
				and have fun!";
			else if ($badge == "Numero Uno")
				$message = "Your badge suggests that you are a top-scorer. 
				You have the maximum developer karma and we are proud of you.
				Keep up the good work and have fun!";
			else if ($badge == "Novice")
				$message = "Your badge suggests that you have not had any 
				contribution to openSUSE so far. We think you can do great 
				work for the community. Work  hard and have fun!";
			else if ($badge == "Strunk & White")
				$message = "Your badge suggests that you have contributed 
				to openSUSE wiki. We encourage you to carry on with the 
				great work you're doing and have fun!";
	...
	function karma_details($username) {
		...
		if ($user instanceof ElggUser) {
			...
			return array("badge"=>$badge,"developer_score"=>array(
				"bugzilla_score"=>$developer_score[0], "build_service_score"
				=>$developer_score[1]), "marketing_score"=>array(
				"twitter_score"=>$marketing_score[0], "planet_opensuse_score"
				=>$marketing_score[1]), "wiki_score" => $wiki_score,
				"karma_last_update"=>$karma_update_time);
		}
		else
